...
const AlbumDetails = () => {
    ...
    const fetchAlbumDetails = async () => {
      try {
        const response = await fetch(
          `http://localhost:3001/album/${albumList[index].id}`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    const fetchPublicReviews = async () => {
      try {
        const response = await fetch(
          `http://localhost:3001/PublicReviews/${albumList[index].id}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  const revIncomplete = () => {
    const rv = !Object.keys(review).every((key) => {
      ...
      return (
        review[key].lyrics &&
        review[key].vocals &&
        review[key].instrumentals &&
        review[key].meaning &&
        review[key].personalOpinion
      );
    });
  ...
  const toggleReviewMode = async () => {
    if (inEditMode) {
      const result = await (
        await fetch("http://localhost:3001/saveReview", {
          mode: "cors",
          method: "post",
